...
const serializeUser = (req, accessToken, refreshToken, profile, done) => {
  ...
  profile = {
    ...
    // Simulate a username by splitting the e-mail address. Genius!
  ...
  if (req.user) {
    ...
    return User.findOne({ username: profile.username }, (findOneErr, existingUser) => {
      // If there is, return an error message. (Account merging not supported)
      if (existingUser) {
        // console.log('Logged in, existingUser by username', existingUser)
  ...
  return User
    ...
    // If returning user, sign in and we are done.
      if (existingUser) {
      // console.log('existingUser by username', existingUser)
        return done(null, existingUser)
      }
    // Else check if there is an existing account with user's username.
      return User.findOne({ username: profile.username }, (findByEmailErr, existingEmailUser) => {
